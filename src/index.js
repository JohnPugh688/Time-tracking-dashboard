// const btn = document.querySelectorAll("#js.timeframe");
// const currentHours = document.querySelectorAll("#js.current");
// const previousHours = document.querySelectorAll("#js.previous");
...
function showInfo(data) {
  for (let i = 0; i < trackerTitle.length; i++) {
    ...
    dailyBtn.addEventListener("click", function () {
      ...
      previousTracker[i].innerHTML =
        "Yesterday - " + String(data[i].timeframes.daily.previous) + "hr/s";
    });
    weeklyBtn.addEventListener("click", function () {
      currentTracker[i].innerHTML = data[i].timeframes.weekly.current;
      previousTracker[i].innerHTML =
        "Last Week - " + String(data[i].timeframes.weekly.previous) + "hr/s";
    });
    monthlyBtn.addEventListener("click", function () {
      currentTracker[i].innerHTML = data[i].timeframes.monthly.current;
      previousTracker[i].innerHTML =
        "Last Month - " + String(data[i].timeframes.monthly.previous) + "hr/s";
    });
  }
